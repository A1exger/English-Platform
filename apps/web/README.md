...
- **Комната урока** (`/lessons/:id/room`): доска + видеопанель рядом. Видео на
  базе LiveKit (`@livekit/components-react`): камера, микрофон, демонстрация
  экрана, чат, сетка участников. Токен берётся из `POST /lessons/:id/join`;
  реальное подключение — при заданных `LIVEKIT_URL`/ключах в бэкенде.
