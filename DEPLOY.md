...
### 8.5. Картинки курсов, попавшие в раздел «Материалы»

Раньше любая загрузка файла становилась материалом в библиотеке — и картинка,
поставленная на страницу урока, обложка курса и картинка в упражнении оказывались
на той же полке, которую преподаватель собирает вручную. Новые загрузки помечают
себя сами (`Material.scope`: `library` или `inline`), и в разделе «Материалы»
видно только `library`. Файлы, загруженные до этого, помечены `library` — эта
команда переносит из них те, что действительно вставлены в контент.

**Сначала посмотреть** (ничего не пишет):

```bash
docker compose -f docker-compose.prod.yml --env-file .env.prod exec api \
  node dist/scripts/reclassify-materials.js
```

Она печатает, сколько файлов в библиотеке и какие из них — медиа курсов, с
указанием, где именно каждый встречается. Если список выглядит правильно:

```bash
docker compose -f docker-compose.prod.yml --env-file .env.prod exec api \
  node dist/scripts/reclassify-materials.js --apply
```

Сами файлы не трогаются и остаются доступными по своим ссылкам — меняется только
то, показывать ли их как материал. Файл, на который ничего не ссылается, остаётся
в библиотеке: это может быть материал, который загрузили, но ещё не прикрепили, и
спрятать его было бы той же ошибкой наоборот.
